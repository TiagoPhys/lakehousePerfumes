...
> **`SELECT * REPLACE` não existe no DBSQL** (testado no warehouse da aula, versão
> 2026.32: `PARSE_SYNTAX_ERROR`). O que existe é `* EXCEPT` — e, com
> `INSERT INTO ... BY NAME`, a ordem das colunas deixa de importar.

```sql
-- pegue um pedido cancelado de verdade e tente regravá-lo com valor diferente de zero
INSERT INTO lakehouse_rotaperfume.silver.pedidos BY NAME
SELECT * EXCEPT (valor_liquido),
       CAST(999.00 AS DECIMAL(18,2)) AS valor_liquido
...
INSERT INTO lakehouse_rotaperfume.silver.pedidos BY NAME
SELECT * EXCEPT (valor_liquido, _processado_em),
       CAST(0.00 AS DECIMAL(18,2))          AS valor_liquido,
...
| `PARSE_SYNTAX_ERROR` no `INSERT` da verificação 5 | `SELECT * REPLACE` não existe no DBSQL | Use `INSERT INTO ... BY NAME` com `SELECT * EXCEPT (col), <novo valor> AS col` |
| Todo "s" minúsculo sumiu da razão social | `'\s+'` numa string SQL chega no regex como `s` | A barra é escape de string: escreva `'\\s+'` ou, mais simples, `' +'` |
